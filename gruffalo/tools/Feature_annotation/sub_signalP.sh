...
if [ -z $SigP_Version ]; then
  WorkDir=$TMPDIR/${SLURM_JOB_USER}_${SLURM_JOBID}/"${InName}_2"
elif [ $SigP_Version == signalp-3.0 ]; then
  WorkDir=$TMPDIR/${SLURM_JOB_USER}_${SLURM_JOBID}/"${InName}_3"
elif [ $SigP_Version == signalp-4.1 ]; then
  WorkDir=$TMPDIR/${SLURM_JOB_USER}_${SLURM_JOBID}/"${InName}_4"
elif [ $SigP_Version == signalp-5.0 ]; then
  WorkDir=$TMPDIR/${SLURM_JOB_USER}_${SLURM_JOBID}/"${InName}_5"
fi
...
if [ -z $SigP_Version ]; then
  ...
  Tarball=$(ls /projects/oldhome/armita/prog/signalP/signalp-2.0/signalp-2.0.Linux.tar.Z)
  cp $Tarball .
  tar -zxf *.tar.Z
  # sed -i '2s/^/set -x\n/' signalp-2.0/signalp
  ...
  echo "Installing SignalP"
  Tarball=$(ls /projects/oldhome/armita/prog/signalP/signalp-3.0/signalp-3.0.Linux.tar.Z)
  cp $Tarball .
  tar -zxf *.tar.Z
  ...
  echo '----------------------------------------------------------------------' >> "$OutFile"_sp.txt
  PathToAnnotateSigP=/home/gomeza/git_repos/scripts/bioinformatics_tools/Feature_annotation
  $PathToAnnotateSigP/sigP_3.0_parser.py --inp_sigP "$OutFile"_sp.txt --out_tab "$OutFile"_sp.tab --out_fasta "$OutFile"_sp.aa --out_neg "$OutFile"_sp_neg.aa -$
elif [ $SigP_Version == signalp-4.1 ]; then
  echo "Running using SignalP version: $SigP_Version"
  ...
  PathToAnnotateSigP=/home/gomeza/git_repos/scripts/bioinformatics_tools/Feature_annotation
  $PathToAnnotateSigP/sigP_5.0_parser.py --inp_sigP "$OutFile"_summary.signalp5 --out_tab "$OutFile"_sp.tab --out_fasta "$OutFile"_sp.aa --out_neg "$OutFile"_s$
  else
