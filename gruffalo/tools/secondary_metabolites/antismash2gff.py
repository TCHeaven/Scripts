...
synthesis genes by antismash. These regions are output as gff features. Also
...
for gb_record in SeqIO.parse(open(conf.inp_antismash,"r"), "genbank"):
    ...
    for feature in gb_record.features:
        if feature.type == "cluster":
            ...
            feature_id = feature.qualifiers["note"][0]
            feature_id = feature_id.replace(' number: ', '_')
            feature_note =  "".join(feature.qualifiers["note"][1:])
            col_9 = ";".join(["ID=" + feature_id, "Notes=" + feature_note])
